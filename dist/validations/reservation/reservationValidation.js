...
exports.reservationCreateSchema = zod_1.z
    .object({
    propertyId: zod_1.z.string().min(1, 'Invalid property ID format'),
    roomTypeId: zod_1.z.string().min(1, 'Invalid room type ID format'),
    startDate: zod_1.z.iso.datetime('Invalid start date format'),
    endDate: zod_1.z.iso.datetime('Invalid end date format'),
    paymentType: zod_1.z.enum(client_1.PaymentType, { message: 'Invalid payment type' }),
    payerEmail: zod_1.z.email('Invalid payer email format').optional()
})
...
exports.reservationUpdateSchema = zod_1.z
    .object({
    startDate: zod_1.z.iso.datetime('Invalid start date format').optional(),
    endDate: zod_1.z.iso.datetime('Invalid end date format').optional(),
    orderStatus: zod_1.z.enum(client_1.Status, { message: 'Invalid order status' }).optional()
})
    .refine(data => {
...
exports.reservationQuerySchema = zod_1.z.object({
    ...
    propertyId: zod_1.z.string().min(1, 'Invalid property ID format').optional(),
    userId: zod_1.z.string().min(1, 'Invalid user ID format').optional(),
    orderStatus: zod_1.z.enum(client_1.Status, { message: 'Invalid order status' }).optional(),
    startDate: zod_1.z.iso.datetime('Invalid start date format').optional(),
    endDate: zod_1.z.iso.datetime('Invalid end date format').optional(),
    sortBy: zod_1.z.enum(['createdAt', 'startDate', 'endDate', 'amount']).default('createdAt').optional(),
    sortOrder: zod_1.z.enum(['asc', 'desc']).default('desc').optional()
});
exports.availabilityCheckSchema = zod_1.z
    .object({
    roomTypeId: zod_1.z.string().min(1, 'Invalid room type ID format'),
    startDate: zod_1.z.string().min(1, 'Invalid start date format'),
    endDate: zod_1.z.iso.datetime('Invalid end date format')
})
    .refine(data => new Date(data.startDate) < new Date(data.endDate), {
    message: 'Start date must be before end date',
...
exports.paymentCreateSchema = zod_1.z.object({
    reservationId: zod_1.z.string().min(1, 'Invalid reservation ID format'),
    amount: zod_1.z.number().min(0, 'Amount must be non-negative'),
    method: zod_1.z.enum(client_1.PaymentType, { message: 'Invalid payment method' }),
    payerEmail: zod_1.z.email('Invalid payer email format')
});
exports.paymentUpdateSchema = zod_1.z.object({
    paymentStatus: zod_1.z.enum(client_1.Status, { message: 'Invalid payment status' }).optional(),
    xenditInvoiceId: zod_1.z.string().optional(),
    paidAt: zod_1.z.iso.datetime('Invalid paid at date format').optional()
});
