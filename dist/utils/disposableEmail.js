...
exports.DisposableEmailGenerator = void 0;
class DisposableEmailGenerator {
    // Generate disposable email khusus untuk disposablemail.com
    static generateDisposableMail(prefix = "prorent") {
    ...
    static generate(prefix = "prorent") {
        const randomId = Math.random().toString(36).substring(2, 12);
        const timestamp = Date.now().toString(36);
    ...
    static isDisposable(email) {
        return this.domains.some((domain) => email.toLowerCase().includes(domain.toLowerCase()));
    }
    // Get instructions for checking emails based on domain
    static getInstructions(email) {
...
3. 📬 Cek inbox untuk email dari ProRent
    ...
    static logRedirect(originalEmail, disposableEmail) {
        console.log(`\n📧 EMAIL REDIRECT:`);
...
exports.DisposableEmailGenerator = DisposableEmailGenerator;
DisposableEmailGenerator.domains = [
    "@disposablemail.com",
    "@tempmail.com",
    "@10minutemail.com",
    "@yopmail.com",
...
exports.default = DisposableEmailGenerator;
