"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const BaseController_1 = __importDefault(require("../BaseController"));
const responseHelper_1 = __importDefault(require("../../helpers/system/responseHelper"));
const roomValidationHelper_1 = __importDefault(require("../../helpers/property/roomValidationHelper"));
const roomErrorHelper_1 = __importDefault(require("../../helpers/property/roomErrorHelper"));
const roomService_1 = __importDefault(require("../../service/property/roomService"));
const logger_1 = __importDefault(require("../../utils/system/logger"));
const property_1 = require("../../constants/controllers/property");
class RoomController extends BaseController_1.default {
    // GET /api/owner/rooms?propertyId= - Get all rooms by property
    getRoomsByProperty(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const userValidation = this.validateUser(req);
                if (!userValidation.isValid) {
                    const errorResponse = roomErrorHelper_1.default.createUnauthorizedError(userValidation.error);
                    responseHelper_1.default.error(res, errorResponse.message, undefined, errorResponse.status);
                    return;
                }
                const propertyValidation = roomValidationHelper_1.default.validatePropertyId(req.query.propertyId);
                if (!propertyValidation.isValid) {
                    const errorResponse = roomErrorHelper_1.default.createValidationError(propertyValidation.error);
                    responseHelper_1.default.error(res, errorResponse.message, undefined, errorResponse.status);
                    return;
                }
                const rooms = yield roomService_1.default.getRoomsByProperty(propertyValidation.cleanId, userValidation.userId);
                responseHelper_1.default.success(res, property_1.ROOM_SUCCESS_MESSAGES.ROOMS_RETRIEVED, rooms);
            }
            catch (error) {
                logger_1.default.error("Error in getRoomsByProperty:", error);
                const errorResponse = roomErrorHelper_1.default.mapError(error, "getRoomsByProperty");
                responseHelper_1.default.error(res, errorResponse.message, undefined, errorResponse.status);
            }
        });
    }
    // POST /api/owner/rooms - Create new room
    createRoom(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const userValidation = this.validateUser(req);
                if (!userValidation.isValid) {
                    const errorResponse = roomErrorHelper_1.default.createUnauthorizedError(userValidation.error);
                    responseHelper_1.default.error(res, errorResponse.message, undefined, errorResponse.status);
                    return;
                }
                const dataValidation = roomValidationHelper_1.default.validateCreateRoomData(req.body);
                if (!dataValidation.isValid) {
                    const errorResponse = roomErrorHelper_1.default.createValidationError("Validation failed");
                    responseHelper_1.default.error(res, errorResponse.message, dataValidation.errors, errorResponse.status);
                    return;
                }
                const newRoom = yield roomService_1.default.createRoom(dataValidation.cleanData, userValidation.userId);
                responseHelper_1.default.success(res, property_1.ROOM_SUCCESS_MESSAGES.ROOM_CREATED, newRoom, 201);
            }
            catch (error) {
                logger_1.default.error("Error in createRoom:", error);
                const errorResponse = roomErrorHelper_1.default.mapError(error, "createRoom");
                responseHelper_1.default.error(res, errorResponse.message, undefined, errorResponse.status);
            }
        });
    }
    // PUT /api/owner/rooms/:id - Update room
    updateRoom(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const userValidation = this.validateUser(req);
                if (!userValidation.isValid) {
                    const errorResponse = roomErrorHelper_1.default.createUnauthorizedError(userValidation.error);
                    responseHelper_1.default.error(res, errorResponse.message, undefined, errorResponse.status);
                    return;
                }
                const idValidation = roomValidationHelper_1.default.validateRoomId(req.params.id);
                if (!idValidation.isValid) {
                    const errorResponse = roomErrorHelper_1.default.createValidationError(idValidation.error);
                    responseHelper_1.default.error(res, errorResponse.message, undefined, errorResponse.status);
                    return;
                }
                const dataValidation = roomValidationHelper_1.default.validateUpdateRoomData(req.body);
                if (!dataValidation.isValid) {
                    const errorResponse = roomErrorHelper_1.default.createValidationError("Validation failed");
                    responseHelper_1.default.error(res, errorResponse.message, dataValidation.errors, errorResponse.status);
                    return;
                }
                const updatedRoom = yield roomService_1.default.updateRoom(idValidation.cleanId, dataValidation.cleanData, userValidation.userId);
                responseHelper_1.default.success(res, property_1.ROOM_SUCCESS_MESSAGES.ROOM_UPDATED, updatedRoom);
            }
            catch (error) {
                logger_1.default.error("Error in updateRoom:", error);
                const errorResponse = roomErrorHelper_1.default.mapError(error, "updateRoom");
                responseHelper_1.default.error(res, errorResponse.message, undefined, errorResponse.status);
            }
        });
    }
    // DELETE /api/owner/rooms/:id - Delete room
    deleteRoom(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const userValidation = this.validateUser(req);
                if (!userValidation.isValid) {
                    const errorResponse = roomErrorHelper_1.default.createUnauthorizedError(userValidation.error);
                    responseHelper_1.default.error(res, errorResponse.message, undefined, errorResponse.status);
                    return;
                }
                const idValidation = roomValidationHelper_1.default.validateRoomId(req.params.id);
                if (!idValidation.isValid) {
                    const errorResponse = roomErrorHelper_1.default.createValidationError(idValidation.error);
                    responseHelper_1.default.error(res, errorResponse.message, undefined, errorResponse.status);
                    return;
                }
                yield roomService_1.default.deleteRoom(idValidation.cleanId, userValidation.userId);
                responseHelper_1.default.success(res, property_1.ROOM_SUCCESS_MESSAGES.ROOM_DELETED, { id: idValidation.cleanId });
            }
            catch (error) {
                logger_1.default.error("Error in deleteRoom:", error);
                const errorResponse = roomErrorHelper_1.default.mapError(error, "deleteRoom");
                responseHelper_1.default.error(res, errorResponse.message, undefined, errorResponse.status);
            }
        });
    }
}
exports.default = new RoomController();
