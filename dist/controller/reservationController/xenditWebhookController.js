...
const handleXenditInvoiceCallback = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (!config_1.XENDIT_WEBHOOK_TOKEN) {
        ...
        return res
            .status(500)
            .send(reservationMessages_1.RESERVATION_ERROR_MESSAGES.WEBHOOK_TOKEN_MISSING);
    }
    if (!signature) {
        console.warn("Missing Xendit-Signature header.");
        return res
            .status(400)
            .send(reservationMessages_1.RESERVATION_ERROR_MESSAGES.MISSING_SIGNATURE_HEADER);
    }
    if (!rawBody) {
        console.warn("Missing request body.");
        return res
            .status(400)
            .send(reservationMessages_1.RESERVATION_ERROR_MESSAGES.MISSING_REQUEST_BODY);
    }
    ...
    try {
        ...
        if (!trusted) {
            ...
            return res
                .status(401)
                .send(reservationMessages_1.RESERVATION_ERROR_MESSAGES.INVALID_WEBHOOK_SIGNATURE);
        }
        ...
        return res
            .status(500)
            .send(reservationMessages_1.RESERVATION_ERROR_MESSAGES.SIGNATURE_VERIFICATION_ERROR);
    }
    ...
    try {
        ...
        return res
            .status(400)
            .send(reservationMessages_1.RESERVATION_ERROR_MESSAGES.INVALID_JSON_PAYLOAD);
    }
    ...
    if (!xenditInvoiceId) {
        ...
        return res
            .status(400)
            .send(reservationMessages_1.RESERVATION_ERROR_MESSAGES.MISSING_INVOICE_ID_PAYLOAD);
    }
    ...
    try {
        ...
        if (!paymentRecord) {
            ...
            return res
                .status(200)
                .send(reservationMessages_1.RESERVATION_ERROR_MESSAGES.INVOICE_ID_NOT_FOUND);
        }
        ...
        return res
            .status(200)
            .send(reservationMessages_1.RESERVATION_SUCCESS_MESSAGES.WEBHOOK_PROCESSED_SUCCESSFULLY);
    }
    catch (error) {
        console.error("Error processing Xendit callback:", error);
        return res
            .status(200)
            .send(reservationMessages_1.RESERVATION_ERROR_MESSAGES.WEBHOOK_ERROR_LOGGED);
    }
});
