// src/templates/report/ownerReport.ts
import ExcelJS from 'exceljs';
import * as ReportInterface from '../../interfaces/report/reportCustomInterface';
import { addAvailabilitySheet, addReservationsSheet, truncateSheetName, formatDate } from './helper';
...
export function addFullOwnerSheets (workbook: ExcelJS.Workbook, report: ReportInterface.DashboardReportResponse) {
   // Add a sheet for each property
   report.properties.forEach(p => {
      const sheetName = truncateSheetName(p.property.name);
      const sheet = workbook.addWorksheet(sheetName);

      let currentRow = 1; // Keep track of the current row

      // --- Property Summary Title ---
      const propertySummaryTitleRow = sheet.addRow([ `${p.property.name} Property Summary` ]);
      sheet.mergeCells(propertySummaryTitleRow.number, 1, propertySummaryTitleRow.number, MAIN_TABLE_COLUMNS);
      propertySummaryTitleRow.getCell(1).font = { bold: true, size: 14 };
      propertySummaryTitleRow.getCell(1).alignment = { horizontal: 'center', vertical: 'middle' };
      currentRow++; // Move to next row
      sheet.addRow([]); // Spacer row
      currentRow++;
      // --- Property Details ---
      sheet.addRow([ 'Name', p.property.name ]);
      currentRow++;
      // --- Address Row with Dynamic Merging ---
      const addressLabelCell = sheet.getCell(currentRow, 1);
      addressLabelCell.value = 'Address';
      const addressValue = p.property.address || '-';
      ...
      const columnsToMerge = Math.max(2, Math.min(estimatedColumnsNeeded, MAIN_TABLE_COLUMNS)); // Merge at least B:C if needed

      if (columnsToMerge > 1) {
         sheet.mergeCells(currentRow, 2, currentRow, columnsToMerge); // Merge B to column index
      }
      // Align address value
      addressValueCell.alignment = { vertical: 'top', wrapText: true }; // Allow wrapping if needed
      currentRow++;

      sheet.addRow([ 'City', p.property.city || '-' ]);
      currentRow++;
      sheet.addRow([ 'Province', p.property.province || '-' ]);
      currentRow++;
      sheet.addRow([ 'Rental Type', p.property.rentalType ]);
      currentRow++;
      sheet.addRow([]); // Spacer row
      currentRow++;
      ...
      sheet.addRow([ 'Confirmed', p.summary.counts.CONFIRMED ]);
      currentRow++;
      sheet.addRow([ 'Pending Payment', p.summary.counts.PENDING_PAYMENT ]);
      currentRow++;
      sheet.addRow([ 'Pending Confirmation', p.summary.counts.PENDING_CONFIRMATION ]);
      currentRow++;
      sheet.addRow([ 'Cancelled', p.summary.counts.CANCELLED ]);
      currentRow++;
      const actualRevenueRow = sheet.addRow([ 'Actual Revenue', p.summary.revenue.actual ]);
      actualRevenueRow.getCell(2).numFmt = '"Rp"#,##0';
      currentRow++;
      const projectedRevenueRow = sheet.addRow([ 'Projected Revenue', p.summary.revenue.projected ]);
      projectedRevenueRow.getCell(2).numFmt = '"Rp"#,##0';
      currentRow++;
      sheet.addRow([ 'Average Revenue', p.summary.revenue.average ]); // Assuming average doesn't need currency
      currentRow++;
      sheet.addRow([ 'Total Room Types', p.summary.totalRoomTypes ]);
      currentRow++;
      sheet.addRow([]); // Spacer row
      currentRow++;
   ...
   // Add the separate "All Reservations" sheet at the end (optional, as per original)
   // Pass the flag to include property/room type columns
   addReservationsSheet(workbook, report, 'All Reservations', undefined, undefined, true);
}
