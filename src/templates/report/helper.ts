// src/templates/report/helper.ts
...
export function addReservationsSheet (
   workbook: ExcelJS.Workbook,
   report: ReportInterface.DashboardReportResponse,
   title: string,
   propertyId?: string,
   roomTypeId?: string,
   includePropertyRoomType: boolean = false // New flag
) {
   ...
   const reservationData = report.properties
      ...
      .flatMap(p =>
         p.roomTypes
            ...
            .map(rt =>
               rt.reservationListItems.map(r => ({ reservation: r, property: p.property, roomType: rt.roomType }))
            )
            .flat()
      );
...
export function addAvailabilitySheet (
   sheet: ExcelJS.Worksheet,
   roomTypes: ReportInterface.RoomTypeWithAvailability[],
   startRow = 1
) {
   const dates = roomTypes[0]?.availability.dates.map(d => d.date) || [];
   if (dates.length === 0) {
      return;
   }
...
export function addFiltersSheet (workbook: ExcelJS.Workbook, filters: Partial<ReportInterface.ReportFilters>) {
...
export function autoFitColumns (workbook: ExcelJS.Workbook) {
