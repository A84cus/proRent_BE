import { UserWithProfile } from '../../interfaces/email/email.interface';
import { LOGO_URL } from '../../config';

// Version 1: Canceled by User
export const createBookingCancellationByUserTemplate = (user: UserWithProfile, bookingDetails: any): string => {
   ...
   return `
    ...
    <body>
      ...
      <div class="content">
        <p>Hi ${fullName},</p>
        <p>We've received your cancellation request for reservation <strong>${
           bookingDetails.id
        }</strong> and have processed it successfully.</p>
...
export const createBookingCancellationByOwnerTemplate = (user: UserWithProfile, bookingDetails: any): string => {
   const firstName = user.profile?.firstName || user.profile?.lastName || 'Valued Guest';
   const fullName =
      user.profile?.firstName && user.profile?.lastName
         ? `${user.profile.firstName} ${user.profile.lastName}`
   ...
   return `
    ...
    <body>
      ...
      <div class="content">
        <p>Hi ${fullName},</p>
        <p>We regret to inform you that your reservation <strong>${bookingDetails.id}</strong> for <strong>${
      bookingDetails.propertyName
   }</strong> has been cancelled by the ${bookingDetails.propertyName} organizer.</p>
