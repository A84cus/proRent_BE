...
class AvailabilityRepository {
   ...
   async findRoomAvailabilityByMonth (roomId: string, year: number, month: number): Promise<Availability[]> {
   ...
   async getExistingAvailability (roomId: string, dates: Date[]): Promise<Availability[]> {
      return prisma.availability.findMany({
         where: {
            roomId,
            date: {
   ...
   async getPeakRates (roomTypeId: string, startDate: Date, endDate: Date): Promise<any[]> {
   ...
   async findRoomTypeAvailabilityByMonth (roomTypeId: string, year: number, month: number): Promise<Availability[]> {
      const startDate = new Date(year, month - 1, 1); // month is 0-indexed in JS Date
      const endDate = new Date(year, month, 0, 23, 59, 59); // Last day of the month
