...
class PublicPropertyRepository {
   ...
   async getDailyPricingAndAvailability (propertyId: string, days: number): Promise<any[]> {
      ...
      for (let i = 0; i < days; i++) {
         ...
         const roomTypeData = roomTypes.map(roomType => {
            ...
            const applicablePeakRate = roomType.peakRates.find(
               rate => currentDate >= new Date(rate.startDate) && currentDate <= new Date(rate.endDate)
            );
            ...
            const reservedRooms = roomType.rooms.filter(room =>
               room.reservations.some(
                  res => currentDate >= new Date(res.startDate) && currentDate <= new Date(res.endDate)
               )
            ).length;
