...
import UserOperationsService from "../../service/user/userOperationsService";
import UserControllerHelper from "../../helpers/user/userControllerHelper";
import BaseController from "../BaseController";
import {
  USER_ERROR_MESSAGES,
  USER_SUCCESS_MESSAGES,
} from "../../constants/controllers/user";
class UserController extends BaseController {
  getProfile = async (req: Request, res: Response) => {
    try {
      ...
      return this.handleSuccess(
        res,
        USER_SUCCESS_MESSAGES.PROFILE_RETRIEVED,
        profileData
      );
    } catch (error) {
  ...
  updateProfile = async (req: Request, res: Response) => {
    try {
      ...
      return this.handleSuccess(
        res,
        USER_SUCCESS_MESSAGES.PROFILE_UPDATED,
        updatedProfile
      );
    } catch (error) {
  ...
  private async processPasswordChange(
    ...
    return this.handleSuccess(res, USER_SUCCESS_MESSAGES.PASSWORD_CHANGED);
  }
  ...
  private async processAvatarUpload(
    ...
    return this.handleSuccess(
      res,
      USER_SUCCESS_MESSAGES.AVATAR_UPDATED,
      avatarResponse
    );
  }
  ...
  reverifyEmail = async (req: Request, res: Response) => {
    try {
      const userValidation = UserControllerHelper.validateUserRequest(req);
      if (!userValidation.isValid) {
        return this.handleAuthError(res, userValidation.error!);
      ...
      return this.handleSuccess(
        res,
        USER_SUCCESS_MESSAGES.EMAIL_VERIFICATION_SENT
      );
    } catch (error) {
