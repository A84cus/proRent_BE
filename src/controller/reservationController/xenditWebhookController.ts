...
export const handleXenditInvoiceCallback = async (
  req: Request,
  res: Response
) => {
  ...
  if (!XENDIT_WEBHOOK_TOKEN) {
    ...
    return res
      .status(500)
      .send(RESERVATION_ERROR_MESSAGES.WEBHOOK_TOKEN_MISSING);
  }
  ...
  if (!signature) {
    ...
    return res
      .status(400)
      .send(RESERVATION_ERROR_MESSAGES.MISSING_SIGNATURE_HEADER);
  }
  ...
  if (!rawBody) {
    ...
    return res
      .status(400)
      .send(RESERVATION_ERROR_MESSAGES.MISSING_REQUEST_BODY);
  }
  ...
  try {
    ...
    if (!trusted) {
      ...
      return res
        .status(401)
        .send(RESERVATION_ERROR_MESSAGES.INVALID_WEBHOOK_SIGNATURE);
    }
    ...
    return res
      .status(500)
      .send(RESERVATION_ERROR_MESSAGES.SIGNATURE_VERIFICATION_ERROR);
  }
  ...
  try {
    ...
    return res
      .status(400)
      .send(RESERVATION_ERROR_MESSAGES.INVALID_JSON_PAYLOAD);
  }
  ...
  if (!xenditInvoiceId) {
    ...
    return res
      .status(400)
      .send(RESERVATION_ERROR_MESSAGES.MISSING_INVOICE_ID_PAYLOAD);
  }
  ...
  try {
    ...
    if (!paymentRecord) {
      console.warn(
        `Payment record not found for Xendit invoice ID: ${xenditInvoiceId}`
      );
      // Acknowledge receipt even if not found to prevent retries for unknown invoices.
      return res
        .status(200)
        .send(RESERVATION_ERROR_MESSAGES.INVOICE_ID_NOT_FOUND);
    }

    console.log(
      `Found payment record ${paymentRecord.id} for invoice ${xenditInvoiceId}`
    );

    // --- b. Map Xendit Status to Internal Status ---
    const internalPaymentStatus =
      mapXenditInvoiceStatusToInternal(invoiceStatus);
    console.log(
      `Mapped Xendit status '${invoiceStatus}' to internal status '${internalPaymentStatus}'`
    );
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.payment.update({
        ...
        data: {
          ...
          paidAt: callbackData.paidAt
            ? new Date(callbackData.paidAt)
            : paymentRecord.paidAt,
        },
      });
      console.log(
        `Updated payment record ${paymentRecord.id} status to ${internalPaymentStatus}`
      );
      ...
      if (newReservationStatus && paymentRecord.reservationId) {
        ...
        console.log(
          `Updated reservation ${paymentRecord.reservationId} status to ${newReservationStatus}`
        );
    ...
    console.log(
      `Successfully processed Xendit callback for invoice ${xenditInvoiceId}`
    );
    // --- 6. Acknowledge Success ---
    return res
      .status(200)
      .send(RESERVATION_SUCCESS_MESSAGES.WEBHOOK_PROCESSED_SUCCESSFULLY);
  } catch (error: any) {
    console.error("Error processing Xendit callback:", error);

    return res
      .status(200)
      .send(RESERVATION_ERROR_MESSAGES.WEBHOOK_ERROR_LOGGED);
  }
};
