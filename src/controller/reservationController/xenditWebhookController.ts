...
export const handleXenditInvoiceCallback = async (req: Request, res: Response) => {
   ...
   if (!XENDIT_WEBHOOK_TOKEN) {
      ...
      return res.status(500).send('Server configuration error: Webhook token missing.');
   }

   if (!signature) {
      console.warn('Missing Xendit-Signature header.');
      return res.status(400).send('Bad Request: Missing Xendit-Signature header.');
   }
   ...
   try {
      ...
      console.log('Parsed callback data:', { id: callbackData.id, status: callbackData.status });
   ...
   if (!xenditInvoiceId) {
      ...
      return res.status(400).send('Bad Request: Missing invoice ID in payload.');
   }
   ...
   try {
      ...
      if (!paymentRecord) {
         ...
         return res.status(200).send('OK - Invoice ID not found locally.');
      }
      ...
      return res.status(200).send('OK - Error logged internally.');
   }
};
