...
const etherealConfig = {
  ...
  from: THIRD_PARTY_CONFIG.SMTP_FROM || "noreply@prorent.com",
};

export const emailConfig: EmailConfig = THIRD_PARTY_CONFIG.USE_GMAIL
  ? {
      host: "smtp.gmail.com",
      port: 587,
  ...
  : THIRD_PARTY_CONFIG.USE_ETHEREAL
  ? {
      host: etherealConfig.host,
      port: etherealConfig.port,
      secure: etherealConfig.secure,
      user: etherealConfig.user,
      pass: etherealConfig.pass,
      from: etherealConfig.from,
      frontendUrl: THIRD_PARTY_CONFIG.BASE_FE_URL || "http://localhost:3000",
    }
  : {
...
export const createEmailTransporter = () => {
  ...
  if (THIRD_PARTY_CONFIG.USE_GMAIL) {
  ...
  } else if (THIRD_PARTY_CONFIG.USE_ETHEREAL) {
    transporter = nodemailer.createTransport({
      host: etherealConfig.host,
      port: etherealConfig.port,
      secure: etherealConfig.secure,
