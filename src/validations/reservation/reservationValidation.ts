...
export const reservationCreateSchema = z
   .object({
      propertyId: z.string().min(1, 'Invalid property ID format'),
      roomTypeId: z.string().min(1, 'Invalid room type ID format'),
      startDate: z.iso.datetime('Invalid start date format'),
      endDate: z.iso.datetime('Invalid end date format'),
      paymentType: z.enum(PaymentType, { message: 'Invalid payment type' }),
      payerEmail: z.email('Invalid payer email format').optional()
   })
...
export const reservationUpdateSchema = z
   .object({
      ...
      orderStatus: z.enum(Status, { message: 'Invalid order status' }).optional()
   })
   .refine(
      data => {
...
export const reservationQuerySchema = z.object({
   ...
   propertyId: z.string().min(1, 'Invalid property ID format').optional(),
   userId: z.string().min(1, 'Invalid user ID format').optional(),
   orderStatus: z.enum(Status, { message: 'Invalid order status' }).optional(),
   startDate: z.iso.datetime('Invalid start date format').optional(),
   endDate: z.iso.datetime('Invalid end date format').optional(),
   sortBy: z.enum([ 'createdAt', 'startDate', 'endDate', 'amount' ]).default('createdAt').optional(),
...
export const paymentCreateSchema = z.object({
   reservationId: z.string().min(1, 'Invalid reservation ID format'),
   amount: z.number().min(0, 'Amount must be non-negative'),
   method: z.enum(PaymentType, { message: 'Invalid payment method' }),
   payerEmail: z.email('Invalid payer email format')
});

export const paymentUpdateSchema = z.object({
   paymentStatus: z.enum(Status, { message: 'Invalid payment status' }).optional(),
   xenditInvoiceId: z.string().optional(),
   paidAt: z.iso.datetime('Invalid paid at date format').optional()
});
