...
import { getDefaultPeriodParams } from './cronjobHelperService';
...
export function getCurrentYearAndPreviousMonthInfo (
   periodType: string,
   periodKey: string,
   year: number
): { isCurrentYearCalculation: boolean; previousMonthKey: string | null } {
   const now = new Date();
...
export function getLastCompletedMonthPeriodKey (): string {
   const now = new Date();
...
export function getCurrentYearPeriodKey (): string {
   const now = new Date();
