...
import { createPaymentProofRejectionTemplate } from '../../templates/email/rejectionPaymentProofEmail';
import {
   createBookingCancellationByOwnerTemplate,
   createBookingCancellationByUserTemplate
} from '../../templates/email/bookingCancelationEmail';
...
class EmailService {
   ...
   async sendBookingRejection (user: UserWithProfile, bookingDetails: BookingDetails): Promise<void> {
      try {
         const htmlContent = createPaymentProofRejectionTemplate(user, bookingDetails);

         await this.sendEmail({
            to: user.email,
            subject: `Your Payment Proof for Booking No.${bookingDetails.id} has been Rejected - ProRent`,
            html: htmlContent
         });
   ...
   async sendBookingCancelationByUser (user: UserWithProfile, bookingDetails: BookingDetails): Promise<void> {
      try {
         const htmlContent = createBookingCancellationByUserTemplate(user, bookingDetails);

         await this.sendEmail({
            to: user.email,
            subject: 'Booking Cancelation - ProRent',
            html: htmlContent
         });
   ...
   async sendBookingCancelationByOwner (user: UserWithProfile, bookingDetails: BookingDetails): Promise<void> {
      try {
         const htmlContent = createBookingCancellationByOwnerTemplate(user, bookingDetails);
