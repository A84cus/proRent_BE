...
async function validateBooking(data: any) {
  ...
  const targetRoomTypeId = await resolveTargetRoomTypeId(
    data.propertyId,
    data.roomTypeId
  );
  ...
  if (!isAvailable) {
    throw new Error(
      "The selected accommodation type is not available for the chosen dates."
    );
  }
...
async function handleXenditPostProcessing(
  ...
  try {
    ...
    console.error(
      "Error creating Xendit invoice after reservation:",
      xenditError
    );
    throw new Error(
      `Reservation created, but Xendit payment setup failed: ${xenditError.message}`
    );
  }
}
...
export async function createReservation(input: unknown) {
  ...
  const { reservation, paymentRecordId } = await executeReservationTransaction(
    data,
    validationData
  );
...
async function findAndValidateReservation(
  reservationId: string,
  userId: string
) {
  ...
  if (reservation.orderStatus !== Status.PENDING_PAYMENT) {
    throw new Error(
      "Cancellation is not allowed for reservations that are confirmed or awaiting confirmation."
    );
  }
...
async function updateReservationAndPaymentStatus(
  tx: any,
  reservationId: string
) {
...
async function restoreAvailability(tx: any, reservation: any) {
  if (
    ...
    console.warn(
      `Could not restore availability for reservation ${reservation.id}: Missing RoomTypeId or dates.`
    );
  }
}
