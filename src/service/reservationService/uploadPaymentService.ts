...
async function validateReservationAccess(
  ...
  if (reservation.userId !== userId) {
    throw new Error(
      "Unauthorized: You can only upload proof for your own reservations."
    );
  }

  if (reservation.orderStatus !== Status.PENDING_PAYMENT) {
    throw new Error(
      "Payment proof can only be uploaded for reservations pending payment."
    );
  }

  if (reservation.payment?.method !== "MANUAL_TRANSFER") {
    throw new Error(
      "Payment proof upload is only allowed for manual transfer payments."
    );
  }
...
async function uploadFileToStorage(file: Express.Multer.File): Promise<string> {
  try {
    ...
    throw new Error(
      `Failed to upload payment proof to storage: ${
...
function generateAltText(reservation: any, validationResult: any): string {
  ...
  return (
    ...
    `Payment proof for Invoice No. ${
      reservation.payment?.invoiceNumber || serial
    } reservation `
  );
}
...
export async function uploadPaymentProof(
  ...
  return await prisma.$transaction(async (tx: any) => {
    ...
    await createPaymentProofRecords(
      tx,
      reservation,
      file,
      cloudinaryUrl,
