...
async function runPostRejectionExpiryCheck(
  reservationId: string
): Promise<void> {
  console.log(
    `Checking for expiry after rejecting reservation ${reservationId}...`
  );
...
async function checkFinalReservationStatus(reservationId: string) {
  ...
  if (finalReservationCheck.orderStatus === Status.CANCELLED) {
    console.log(
      `Reservation ${reservationId} was automatically cancelled because it had expired.`
    );
    throw new Error(
      "Reservation was automatically cancelled because it had expired."
    );
  }

  console.log(
    `Reservation ${reservationId} successfully rejected (status PENDING_PAYMENT).`
  );
  return finalReservationCheck; // Optional: return data if needed elsewhere
}

async function findAndValidateReservationForOwner(
  reservationId: string,
  ownerId: string
): Promise<any> {
  await cancelExpiredReservations();
  ...
  if (reservation.Property?.OwnerId !== ownerId) {
    throw new Error(
      "Unauthorized: You can only process reservations for your own properties."
    );
  }
...
export async function rejectReservationByOwner(
  reservationId: string,
  ownerId: string
) {
  const reservation = await findAndValidateReservationForOwner(
    reservationId,
    ownerId
  );
  try {
    ...
    if (error.code === "P2025") {
      console.error(
        `Failed to reject reservation ${reservationId}: Record not found or status mismatch.`
      );
      throw new Error(
        "Reservation could not be rejected. It might have expired and been automatically cancelled."
      );
    }
...
export async function confirmReservationByOwner(
  reservationId: string,
  ownerId: string
) {
  const reservation = await findAndValidateReservationForOwner(
  ...
  try {
    ...
    console.log(
      `Reservation ${reservationId} confirmed by owner ${ownerId}. Status changed to CONFIRMED.`
    );
    try {
    ...
    if (error.code === "P2025") {
      console.error(
        `Failed to confirm reservation ${reservationId}: Record not found or status mismatch.`
      );
      throw new Error(
        "Reservation could not be confirmed. It might have expired and been automatically cancelled."
      );
    }
