...
export async function queryReservations(options: ReservationQueryOptions) {
  ...
  return await executeReservationQuery(
    whereConditions,
    orderBy,
    includeFields,
    skip,
...
export async function getUserReservations(
  userId: string,
  options: Omit<ReservationQueryOptions, "userId"> = {}
) {
...
export async function getReservationWithPayment(reservationId: string) {
  try {
    ...
    console.error(
      `Error fetching reservation with payment for ID ${reservationId}:`,
      error
    );
